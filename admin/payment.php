                        ...
                        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                            <div class="breadcome-list">
                                <div class="row">
                                    ...
                                    <div class="col-lg-6 col-md-6 col-sm-6 col-xs-6 ">
                                        <?php
                                        $fetchMax = "SELECT `added_on` FROM `payment` WHERE `added_on`=(SELECT MAX(added_on) FROM `payment`)";
                                        $fetchMaxResult = mysqli_query($connect, $fetchMax);
                                        $maxData = mysqli_fetch_assoc($fetchMaxResult);
                                        $maxDate = $maxData['added_on'];
                                        $maxDate = date('Y-m-d', strtotime($maxDate) + 83200);


                                        $fetchMin = "SELECT `added_on` FROM `payment` WHERE `added_on`=(SELECT MIN(added_on) FROM `payment`)";
                                        $fetchMinResult = mysqli_query($connect, $fetchMin);
                                        $minData = mysqli_fetch_assoc($fetchMinResult);
                                        $minDate = $minData['added_on'];
                                        $minDate = date('Y-m-d', strtotime($minDate) - 83200);
                                        ?>
                                        <form class="divaln" action="./gen_payment.php" method="post" enctype="multipart/form-data">
